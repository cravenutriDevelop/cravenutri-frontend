    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
        ...
        <motion.div variants={imageContainerVariants} className="space-y-4">
          {/* Main image with zoom effect and sold out overlay */}
          <div className="relative group overflow-hidden rounded-2xl bg-gray-50 shadow-lg">
        ...
        <motion.div variants={infoContainerVariants} className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">SKU: {productData._id.slice(-8).toUpperCase()}</p>
          </div>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            {/* {!isOutOfStock && productData.stock > 0 && (
          ...
          {productData.nutritionInfo && (
            <motion.div
              ...
              className="space-y-3"
            >
              <h3 className="text-lg font-semibold text-gray-800">Nutrition Facts (per serving)</h3>
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                {Object.entries(productData.nutritionInfo).map(([key, value]) => (
                  <div
                    key={key}
          ...
          >
            ...
            <p className="flex items-center gap-2">
              <svg className="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0v8m0-8l-8 4-8-4m8 4v8m0-8l8 4-8 4-8-4 8-4z" />
              </svg>
      ...
      >
        ...
        {productData.whyYouWillLoveIt?.length > 0 && (
          <div>
            <h3 className="text-xl font-semibold text-gray-800 mb-2">Why You'll Love It</h3>
            <ul className="list-disc list-inside text-gray-600 space-y-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-gray-700">
              {productData.flavor && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <span className="font-medium">Flavor:</span> {productData.flavor}
                </div>
              )}
              {productData.netWeight && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <span className="font-medium">Net Weight:</span> {productData.netWeight}
                </div>
              )}
              {productData.servingSize && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <span className="font-medium">Serving Size:</span> {productData.servingSize}
                </div>
              )}
              {productData.servingsPerContainer && (
                <div className="bg-gray-50 p-3 rounded-lg">
                  <span className="font-medium">Servings Per Container:</span> {productData.servingsPerContainer}
                </div>
              )}
            </div>
          </div>
        ...
        {(productData.storageInstructions || productData.shelfLife) && (
          <div>
            <h3 className="text-xl font-semibold text-gray-800 mb-2">Storage & Shelf Life</h3>
            <div className="space-y-2 text-gray-700">
              {productData.storageInstructions && (
                <p>
                  <span className="font-medium">Storage:</span> {productData.storageInstructions}
                </p>
              )}
              {productData.shelfLife && (
                <p>
                  <span className="font-medium">Shelf Life:</span> {productData.shelfLife}
                </p>
              )}
            </div>
          </div>
